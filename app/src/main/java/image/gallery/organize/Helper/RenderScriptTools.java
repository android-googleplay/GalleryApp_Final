...
import com.gallery.photo.album.photomanager.ScriptC_convolution;
...
public class RenderScriptTools {
     ...
     * @param kernel the kernel to use for convolution
     */
    public static Bitmap applyConvolution(final Bitmap bmp, final RenderScript rs, final int kernelWidth, final int kernelHeight, final float[] kernel) {
        ...
        Allocation pixels = Allocation.createFromBitmap(rs, res);
        Allocation output = Allocation.createTyped(rs,input.getType());

        script.set_pixels(pixels);

        script.set_width(res.getWidth());
        script.set_height(res.getHeight());
